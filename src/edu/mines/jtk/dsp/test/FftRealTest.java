...
public class FftRealTest extends TestCase {
  ...
  public void test1() {
    ...
    for (int n=2; n<nmax; ++n) {
      ...
      int nw = nfft/2+1;
      float[] c1 = Cap.zero(nw);
      float[] r1 = c1;
      r1[1] = 1.0f;
      float[] rx = Rap.copy(r1);
      float[] cx = rx;
      fft.realToComplex(1,rx,cx);
      float ra = 0.0f;
      float rb = 2.0f*FLT_PI/(float)nfft;
      float[] amp = Rap.fill(1.0f,nw);
      float[] phs = Rap.ramp(ra,rb,nw);
  ...
  public void test12() {
    ...
    for (int n2=2; n2<n2max; ++n2) {
      ...
      for (int n1=2; n1<n1max; ++n1) {
        ...
        int nw = n1fft/2+1;
        float[][] c1 = Cap.zero(nw,n2fft);
        float[][] r1 = c1;
        r1[1][1] = 1.0f;
        float[][] rx = Cap.copy(r1);
        float[][] cx = rx;
        ...
        float[][] amp = Rap.fill(1.0f,nw,n2fft);
        float[][] phs = Rap.ramp(ra,rb1,rb2,nw,n2fft);
        float[][] cc = Cap.polar(amp,phs);
  ...
  public void test21() {
    ...
    for (int n2=2; n2<n2max; ++n2) {
      ...
      for (int n1=2; n1<n1max; ++n1) {
        ...
        float rb1 = 2.0f*FLT_PI/(float)n1fft;
        float rb2 = 2.0f*FLT_PI/(float)n2fft;
        float[][] amp = Rap.fill(1.0f,n1fft,nw);
        float[][] phs = Rap.ramp(ra,rb1,rb2,n1fft,nw);
        float[][] cc = Cap.polar(amp,phs);
  ...
  public void test1Random() {
    ...
    for (int n=2; n<nmax; ++n) {
      ...
      FftReal fft = new FftReal(nfft);
      int nw = nfft/2+1;
      float[] rr = Rap.rand(nfft);;
      float[] rx = Rap.copy(rr);
      float[] cy = Cap.zero(nw);
  ...
  public void test12Random() {
    ...
    for (int n2=2; n2<n2max; ++n2) {
      ...
      for (int n1=2; n1<n1max; ++n1) {
        ...
        FftReal fft1 = new FftReal(n1fft);
        int nw = n1fft/2+1;
        float[][] rr = Rap.rand(n1fft,n2);;
        float[][] rx = Rap.copy(rr);
        float[][] cy = Cap.zero(nw,n2fft);
  ...
  public void test21Random() {
    ...
    for (int n2=2; n2<n2max; ++n2) {
      ...
      for (int n1=2; n1<n1max; ++n1) {
        int n1fft = FftComplex.nfftSmall(n1);
        FftComplex fft1 = new FftComplex(n1fft);
        float[][] rr = Rap.rand(n1,n2fft);;
        float[][] rx = Rap.copy(rr);
        float[][] cy = Cap.zero(n1fft,nw);
